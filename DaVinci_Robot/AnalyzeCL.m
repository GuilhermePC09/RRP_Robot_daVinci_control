...
% AnalyzeCL Realiza a analise refinada de malha fechada de um regulador
%
% Inputs: A, B, C, D, E   - Matrizes de espaco de estados do sistema
%         K               - Matriz de ganhos de realimentacao (u = -Kx)
%         controller_name - String com o nome do controlador ('LQR' ou 'PP')
...
% 1) GRÁFICO DE NOVOS POLOS COMPARANDO COM POLOS ANTIGOS
...
% 2) CENÁRIO REGULADOR DE POSIÇÃO (RESPOSTA À CONDIÇÃO INICIAL)

% Configuração das malhas para condição inicial
sys_ma_init = ss(A, zeros(size(B)), C, zeros(size(D)));
sys_cl_init = ss(A - B * K, zeros(size(B)), C, zeros(size(D)));

t = 0:0.001:2;  % Vetor de tempo 
...
% 2.1) Gráfico de posicao (comparando MF com MA)
...
% 2.2) Gráfico de esforços de controle (u = -Kx)
...
%  3) CENÁRIO DE DISTÚRBIO SENOIDAL
t = 0:0.001:5;  % Vetor de tempo 

% Definição do sinal de respiração: 0.5 Hz, amplitude de 5 mm (0.005 m)
f_respiracao = 0.5;
omega_w = 2 * pi * f_respiracao;
w_senoidal = 0.005 * sin(omega_w * t);

% Sistemas para análise de distúrbio (Entrada: w, Saída: y)
...
% 3.1) Gráfico de posição (comparando MF com MA)
...
% 3.2) Gráfico de esforços de controle
...
%  4) DIAGRAMA DE BODE MF

% Avalia a resposta em frequência do canal do disturbio (w) para as juntas (y)
